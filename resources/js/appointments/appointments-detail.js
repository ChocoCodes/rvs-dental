document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!list || !detailPanel || !detailContent || !baseUrl) return;
    ...
    const showDetail = async (id) => {
        ...
        detailPanel.classList.add('w-96', 'opacity-100');
        ...
        try {
            ...
            detailContent.innerHTML = `
                <div class="flex flex-col h-full w-full bg-secondary bg-opacity-30 overflow-y-auto">
                    <div class="p-6 border-b border-edge">
                        <div class="flex justify-between items-start mb-2">
                            <h2 class="text-2xl font-black text-gray-900 uppercase leading-tight">
                                ${data.day_name || 'THU'} ${data.formatted_date || '23/03/25'} ${data.time || '14:00'}
                            </h2>
                            <button id="close-detail" class="text-danger hover:scale-110 transition-transform">
                                <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/>
                                </svg>
                            </button>
                        </div>
                        <p class="text-md font-medium text-gray-700">${data.patient_name} | ${data.dentist_name}</p>
                    </div>
                    ...
                    <div class="px-6 py-6 border-t border-edge mt-auto">
                        <div class="flex gap-4 items-center">
                            <div class="w-20 h-20 bg-white border border-edge"></div>
                            <div class="flex-1">
                                <h3 class="text-lg font-bold text-gray-900">${data.patient_name}</h3>
                                <div class="grid grid-cols-2 gap-x-2 gap-y-1 mt-1 text-[11px] text-gray-600">
                                    <p><span class="font-semibold text-gray-900">Age:</span> ${data.age || '20'} y.o.</p>
                                    <p><span class="font-semibold text-gray-900">P:</span> ${data.phone || '+65 123 456 7890'}</p>
                                    <p><span class="font-semibold text-gray-900">DOB:</span> ${data.dob || 'May/14/2005'}</p>
                                    <p><span class="font-semibold text-gray-900">Email:</span> ${data.email || 'psbenco@gmail.com'}</p>
                                </div>
                            </div>
                        </div>

                        <button class="w-full mt-8 bg-primary hover:bg-opacity-90 text-white py-4 px-6 rounded-sm font-bold text-sm flex items-center justify-center gap-2 transition-all">
    ...
    const closeDetail = () => {
        ...
        detailPanel.classList.remove('w-96', 'opacity-100');
    };
